'''
code modified from pyod documentation. See https://github.com/yzhao062/pyod/blob/master/pyod/models/knn.py
'''
...
class KNN(DetectorB):
    ...
    def CAD_predict(self, X, init = 0.1, y=None):
        """Predict the anamoly score of data by conformal anaomly detection method
        ----------
        ...
        """
        length = X.shape[0]
        trainX = X[:int(init * length)]
        testX = X[int(init * length):]

        self.fit(trainX)
        pred_scores = np.zeros([X.shape[0], 1])
        for i in range(int(init * length), X.shape[0]):
            x_i = X[i, :]
        ...
        self.decision_scores_ = pred_scores.ravel()
        return self
